...
void solve()
{
   ...
   if(isyao)cout<<"XingYao wins!"<<endl;                          
   else if(isyu)cout<<"XiangYu wins!"<<endl;                                       
   else cout<<"Virus!"<<endl;
}   
int main()
{
    cin>>t;
    while(t--)
    {
       cin>>ci;yao.n[0]=f(ci[0]);yao.c[0]=ci[1];
       cin>>ci;yao.n[1]=f(ci[0]);yao.c[1]=ci[1];
       cin>>ci;yao.n[2]=f(ci[0]);yao.c[2]=ci[1];
       cin>>ci;yu.n[0]=f(ci[0]);yu.c[0]=ci[1];
       cin>>ci;yu.n[1]=f(ci[0]);yu.c[1]=ci[1];
       cin>>ci;yu.n[2]=f(ci[0]);yu.c[2]=ci[1];
       solve();
    }
}   
